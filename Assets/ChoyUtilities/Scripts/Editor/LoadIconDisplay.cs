...
[InitializeOnLoad]
public static class LoadIconDisplay
{
    ...
    {
        if (_window == null)
            _window = Resources.FindObjectsOfTypeAll<EditorWindow>().FirstOrDefault(window => window.GetType().Name == "SceneHierarchyWindow");
    ...
    static void OnHierarchyWindowItemOnGUI(int instanceID, Rect selectionRect)
    {
        ...
        //Check if the gameobject has any component, if null return
        Component[] components = obj.GetComponents<Component>();
        if(components == null || components.Length == 0) return;

        //Get the second-highest arrangement component in the gameobject, if none use transform instead
