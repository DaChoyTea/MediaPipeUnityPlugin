...
namespace Mediapipe.Unity.CoordinateSystem
{
  ...
  public static class ImageCoordinate
  {
    ...
    public static float3 ImageToLocalPoint(int x, int y, int z, float xMin, float xMax, float yMin, float yMax,
                                            int imageWidth, int imageHeight, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false)
    {
      ...
      return new float3(localX, localY, z);
    }
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the image coordinates is mirrored</param>
    public static float3 ImageToPoint(UnityEngine.Rect rectangle, int x, int y, int z,
                                       int imageWidth, int imageHeight, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToLocalPoint(x, y, z, rectangle.xMin, rectangle.xMax, rectangle.yMin, rectangle.yMax, imageWidth, imageHeight, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the image coordinates is mirrored</param>
    public static float3 ImageToPoint(UnityEngine.Rect rectangle, int3 position,
                                       Vector2Int imageSize, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToPoint(rectangle, position.x, position.y, position.z, imageSize.x, imageSize.y, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the image coordinates is mirrored</param>
    public static float3 ImageToPoint(UnityEngine.Rect rectangle, int x, int y,
                                       int imageWidth, int imageHeight, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToLocalPoint(x, y, 0, rectangle.xMin, rectangle.xMax, rectangle.yMin, rectangle.yMax, imageWidth, imageHeight, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the image coordinates is mirrored</param>
    public static float3 ImageToPoint(UnityEngine.Rect rectangle, Vector2Int position, int2 imageSize,
                                       RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToPoint(rectangle, position.x, position.y, imageSize.x, imageSize.y, imageRotation, isMirrored);
    ...
    public static float3 ImageNormalizedToLocalPoint(float normalizedX, float normalizedY, float normalizedZ, float xMin, float xMax, float yMin, float yMax,
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 ImageNormalizedToPoint(UnityEngine.Rect rectangle, float normalizedX, float normalizedY, float normalizedZ,
                                                 float zScale, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToLocalPoint(normalizedX, normalizedY, normalizedZ, rectangle.xMin, rectangle.xMax, rectangle.yMin, rectangle.yMax, zScale, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 ImageNormalizedToPoint(UnityEngine.Rect rectangle, float3 position,
                                                 float zScale, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToPoint(rectangle, position.x, position.y, position.z, zScale, imageRotation, isMirrored);
    ...
    public static float3 ImageNormalizedToPoint(UnityEngine.Rect rectangle, float normalizedX, float normalizedY, float normalizedZ,
                                                 RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToLocalPoint(normalizedX, normalizedY, normalizedZ, rectangle.xMin, rectangle.xMax, rectangle.yMin, rectangle.yMax, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 ImageNormalizedToPoint(UnityEngine.Rect rectangle, Vector3 position, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) => 
      ImageNormalizedToPoint(rectangle, position.x, position.y, position.z, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 ImageNormalizedToPoint(UnityEngine.Rect rectangle, float normalizedX, float normalizedY,
                                                 RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToPoint(rectangle, normalizedX, normalizedY, 0, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 ImageNormalizedToPoint(UnityEngine.Rect rectangle, Vector2 position, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) 
      => ImageNormalizedToPoint(rectangle, position.x, position.y, imageRotation, isMirrored);
    ...
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static Vector3[] ImageToRectVertices(UnityEngine.Rect rectangle, int xMin, int yMin, int width, int height,
                                                int2 imageSize, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, xMin, yMin, width, height, imageSize.x, imageSize.y, imageRotation, isMirrored);
                                                ...
                                                int imageWidth, int imageHeight, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false)
    {
      ...
      var q = quaternion.Euler(new float3(0, 0, (isRotationReversed ? -1 : 1) * math.degrees(rotation)));

      var bottomLeftRel = math.Euler(q) * new float3(-rectWidth / 2, -rectHeight / 2, 0);
      var topLeftRel = math.Euler(q) * new float3(-rectWidth / 2, rectHeight / 2, 0);
    ...
    public static Vector3[] ImageToRectVertices(UnityEngine.Rect rectangle, int xCenter, int yCenter, int width, int height, float rotation,
                                                Vector2Int imageSize, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, xCenter, yCenter, width, height, rotation, imageSize.x, imageSize.y, imageRotation, isMirrored);
                                                          ...
                                                          float rotation, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false)
    {
      ...
      var bottomLeftRel = math.Euler(q) * new float3(-width / 2, -height / 2, 0);
      var topLeftRel = math.Euler(q) * new float3(-width / 2, height / 2, 0);
    ...
    private static Vector3[] GetRectVertices(float3 p, float3 q)
    {
      var leftX = math.min(p.x, q.x);
      var rightX = math.max(p.x, q.x);
      var bottomY = math.min(p.y, q.y);
      var topY = math.max(p.y, q.y);

      var bottomLeft = new float3(leftX, bottomY, 0);
      var topLeft = new float3(leftX, topY, 0);
      var topRight = new float3(rightX, topY, 0);
      var bottomRight = new float3(rightX, bottomY, 0);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float2 GetPoint(this UnityEngine.Rect rectangle, mplt.RelativeKeypoint relativeKeypoint,
      RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false)
    {
      var value = ImageNormalizedToPoint(rectangle, relativeKeypoint.X, relativeKeypoint.Y, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float2 GetPoint(this UnityEngine.Rect rectangle, mptcc.NormalizedKeypoint normalizedKeypoint,
      RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false)
    {
      var value = ImageNormalizedToPoint(rectangle, normalizedKeypoint.x, normalizedKeypoint.y, imageRotation, isMirrored);
      return new float2(value.x, value.y);
    }
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 GetPoint(this UnityEngine.Rect rectangle, NormalizedLandmark normalizedLandmark,
                                   RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToPoint(rectangle, normalizedLandmark.X, normalizedLandmark.Y, normalizedLandmark.Z, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float3 GetPoint(this UnityEngine.Rect rectangle, in mptcc.NormalizedLandmark normalizedLandmark,
                                   RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToPoint(rectangle, normalizedLandmark.x, normalizedLandmark.y, normalizedLandmark.z, imageRotation, isMirrored);
    ...
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, mplt.BoundingBox boundingBox, int imageWidth, int imageHeight,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, boundingBox.Xmin, boundingBox.Ymin, boundingBox.Width, boundingBox.Height, imageWidth, imageHeight, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, mplt.BoundingBox boundingBox, int2 imageSize,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, boundingBox.Xmin, boundingBox.Ymin, boundingBox.Width, boundingBox.Height, imageSize, imageRotation, isMirrored);
    ...
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, mplt.RelativeBoundingBox boundingBox,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToRectVertices(rectangle, boundingBox.Xmin, boundingBox.Ymin, boundingBox.Width, boundingBox.Height, imageRotation, isMirrored);
    ...
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, mptcc.Rect rect, int imageWidth, int imageHeight,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, imageWidth, imageHeight, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, mptcc.Rect rect, int2 imageSize,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, imageSize, imageRotation, isMirrored);
    ...
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, Rect rect, int imageWidth, int imageHeight,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, rect.XCenter, rect.YCenter, rect.Width, rect.Height, rect.Rotation, imageWidth, imageHeight, imageRotation, isMirrored);
    ...
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, Rect rect, Vector2Int imageSize,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageToRectVertices(rectangle, rect.XCenter, rect.YCenter, rect.Width, rect.Height, rect.Rotation, imageSize, imageRotation, isMirrored);
    ...
    public static Vector3[] GetRectVertices(this UnityEngine.Rect rectangle, NormalizedRect normalizedRect,
                                            RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false) =>
      ImageNormalizedToRectVertices(rectangle, normalizedRect.XCenter, normalizedRect.YCenter, normalizedRect.Width, normalizedRect.Height, normalizedRect.Rotation, imageRotation, isMirrored);
    ...
    /// </param>
    /// <param name="isMirrored">Set to true if the original coordinates is mirrored</param>
    public static float2 PointToImageNormalized(this UnityEngine.Rect rectangle, float2 localPosition, RotationAngle imageRotation = RotationAngle.Rotation0, bool isMirrored = false)
    {
      var normalizedX = IsXReversed(imageRotation, isMirrored) ?
          math.unlerp(rectangle.width / 2, -rectangle.width / 2, localPosition.x) :
          math.unlerp(-rectangle.width / 2, rectangle.width / 2, localPosition.x);
      var normalizedY = IsYReversed(imageRotation, isMirrored) ?
          math.unlerp(rectangle.height / 2, -rectangle.height / 2, localPosition.y) :
          math.unlerp(-rectangle.height / 2, rectangle.height / 2, localPosition.y);
      return IsInverted(imageRotation) ? new float2(normalizedY, normalizedX) : new float2(normalizedX, normalizedY);
    }
    ...
    public static bool IsXReversed(RotationAngle rotationAngle, bool isMirrored = false)
    {
      return isMirrored ?
        ...
        rotationAngle is RotationAngle.Rotation90 or RotationAngle.Rotation180;
    }
    ...
    /// </summary>
    public static bool IsInverted(RotationAngle rotationAngle) => rotationAngle == RotationAngle.Rotation90 || rotationAngle == RotationAngle.Rotation270;
  }
}
